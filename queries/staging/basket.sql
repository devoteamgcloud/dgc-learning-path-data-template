...
MERGE
  staging.basket_temp AS T USING (
      ...
      deduplicated_detail AS (
        SELECT
          ...
          STRUCT (
            ...
            ROUND(
              SUM(details.unit_price) / COUNT(details.product_name),
              2
            ) AS unit_price
          ) AS detail
        FROM
    ...
    GROUP BY
      ...
      id_cash_desk,
      purchase_date
  ) AS S ON T.id_basket_header = S.id_basket_header
  AND S.id_cash_desk = T.id_cash_desk
...
CREATE OR REPLACE TABLE
  staging.basket AS (
    ...
    FROM
      ...
      LEFT JOIN `{{ project_id }}.cleaned.basket_header` cleaned_basket_header ON staging_basket_temp.id_basket_header = cleaned_basket_header.id_basket_header
  );
